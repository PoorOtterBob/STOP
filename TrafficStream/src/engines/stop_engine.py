...
class STOPEngine():
    def __init__(self, device, model, optimizer, scheduler, 
                 ...
                 node_training, node_test, node_frechet, node_sood_num, dataloader, 
    ...
    def train_batch(self):
        ...
        for X, label in tqdm(self._dataloader['train_loader'].get_iterator(), desc='Training'):
            ...
            pred_corr, label = self._inverse_transform([pred_corr, label], 0)
            ...
            # loss_base, var_base = torch.var_mean(self._loss_fn(pred_base, label, mask_value))
            # loss_corr, var_corr = torch.var_mean(self._loss_fn(pred_corr, label, mask_value))
            
            # loss_base = self._beta * loss_base + var_base
            # loss_corr = self._beta * loss_corr + var_corr
    ...
    def evaluate(self, mode):
        if mode == 'test':
            ...
            self._logger.info('Calculating spatial side information embedding!')
        ...
        if mode == 'val':
            ...
            if self._checkall:
                for i in range(self.model.horizon):
                    res = compute_all_metrics(preds[:,:1+i,:], labels[:,:1+i,:], mask_value)
                    test_mae.append(res[0])
                    test_mape.append(res[1])
                    test_rmse.append(res[2])

                    res1 = compute_all_metrics(preds1[:,:1+i,:], labels1[:,:1+i,:], mask_value1)
                    test_mae1.append(res1[0])
                    test_mape1.append(res1[1])
                    test_rmse1.append(res1[2])

                    if self._node_sood_num: 
                        res2 = compute_all_metrics(preds2[:,:1+i,:], labels2[:,:1+i,:], mask_value2)
