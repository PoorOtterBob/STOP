...
class moving_avg(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.avg(x.transpose(1, -1)).transpose(1, -1)
        return x
...
class MLP(BaseModel):
    def __init__(self, num_layer, model_dim, prompt_dim, tod_size, kernel_size, **args):
        ...
        self.time_series_emb_layer1 = nn.Conv1d(
            in_channels=self.input_len*(self.input_dim-2), out_channels=self.embed_dim, kernel_size=1, bias=True)
    ...
    def forward(self, history_data, label=None, adj=None):
        ...
        seasonal_init, trend_init = self.decompsition(history_data[..., :-2])
        seasonal_init = seasonal_init.transpose(1, 2).reshape(b, n, -1).transpose(1, -1)
        trend_init = trend_init.transpose(1, 2).reshape(b, n, -1).transpose(1, -1)
