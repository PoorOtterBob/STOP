...
class BaseEngine():
    def __init__(self, device, model, optimizer, scheduler, 
                 node_training, node_test, node_sood_num, adj, 
                 dataloader, scaler, sampler, loss_fn, log_dir, logger, args):
        super().__init__()
        self._device = device
        self.model = model
    ...
    def _inverse_transform(self, tensors, tood=0):
        def inv(tensor):
            return self._scaler[tood].inverse_transform(tensor)
    ...
    def _adj_cal(self, node_order):
        if self._adj is None:
            ...
            adj = normalize_adj_mx(adj, self._adj_type)
    ...
    def save_model(self, save_path):
        ...
        filename = 'final_model_s{}_{}_maxratio{}.pt'.format(self._seed, 
                                                             self._year, 
                                                             self._maxratio, 
                                                             )
    ...
    def train_batch(self, adj=None):
        self.model.train()
        train_loss = []
        train_mape = []
        train_rmse = []
        self._dataloader['train_loader'].shuffle()
        for X, label in tqdm(self._dataloader['train_loader'].get_iterator(), desc='Training'):
            ...
            pred = self.model(X, label, adj)
            pred, label = self._inverse_transform([pred, label], 0)
    ...
    def train(self):
        ...
        adj = self._adj_cal(self._node_training)

        wait = 0
        min_loss = np.inf
        for epoch in range(self._max_epochs):
            t1 = time.time()
            mtrain_loss, mtrain_mape, mtrain_rmse = self.train_batch(adj)
            t2 = time.time()

            v1 = time.time()
            mvalid_loss, mvalid_mape, mvalid_rmse = self.evaluate('val', adj)
            v2 = time.time()
    ...
    def evaluate(self, mode, adj=None):
        if mode == 'test':
            self._logger.info('Start test!')
            self.load_model(self._save_path)
            node_eval = self._node_test
            tood = 0 # self._tood
            adj = self._adj_cal(self._node_test)
        else:
        ...
        labels = []
        with torch.no_grad():
            for X, label in tqdm(self._dataloader[mode + '_loader'].get_iterator(), desc=mode):
                # X (b, t, n, f), label (b, t, n, 1)
                X = X[:, :, node_eval, :]
                label = label[..., node_eval, :]
                X, label = self._to_device(self._to_tensor([X, label]))
                pred = self.model(X, label, adj)
                pred, label = self._inverse_transform([pred, label], tood)
        ...
        # preds = torch.cat(preds, dim=0)
        # labels = torch.cat(labels, dim=0)
        # np.save('/home/mjm/LST/LargeST-main/NeurIPS2024-3075/stgcn.npy', preds.numpy())
        # np.save('/home/mjm/LST/LargeST-main/NeurIPS2024-3075/labels.npy', labels.numpy())
        # sys.exit()
        ...
        if mode == 'val':
            ...
            labels1 = labels[..., :-self._node_sood_num]  if self._node_sood_num else labels
